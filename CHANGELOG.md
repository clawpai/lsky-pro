...
### 新增
- 储存间图片迁移功能：后台「储存管理」可将任意储存的图片一键迁移到另一储存（如本地 → WebDAV），支持 MD5 自动去重、可选删除源文件、断点续传和可调并发。
- 智能图片回源：图片请求在本地找不到文件时，自动按图片记录从所属储存（本地 / WebDAV / S3 等）读取并输出，任意储存间迁移后旧外链保持可用，同时写入本地缓存加速后续访问。
- 社会化登录：兼容聚合 CC 登录接口（AppID / AppKey），支持 QQ、微信、百度等多种方式独立配置、登录和绑定。
- 社会化账号安全管理：支持按登录方式独立绑定、精确解绑和重复身份保护。

### 优化
- 品牌更新为 lskyPai-PRO。
- 软件授权密钥默认值更新为 `lskyPai-PRO-free`，公益版本授权验证保持本地通过，不依赖外部授权服务。
- 后台新增功能界面均为中文。
- WebDAV 图片读取链路性能优化（本地缓存直出）。
- 迁移流程改为“先复制核验、后独立清理源副本”，降低断网、停电、进程中断时的数据风险。

### 修复
- 修复 WebDAV 策略上传的图片外链 404 问题。
- 修复聚合登录回调重复 `type` 导致微信登录失败的问题。
- 修复 QQ 与微信绑定列表显示类型互相覆盖的问题。
- 修复同一聚合驱动下微信绑定覆盖 QQ 绑定的问题。
- 修复第三方账号可能串号、重复绑定和解绑误删其他账号的问题。
- 修复部分环境后台语言不跟随系统设置的问题。
