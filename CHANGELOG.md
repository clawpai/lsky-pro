...
### 新增
- 储存间图片迁移功能：后台「储存管理」可将任意储存的图片一键迁移到另一储存（如本地 → WebDAV），支持 md5 自动去重、可选删除源文件、断点续传，迁移后原外链保持不变
- 智能图片回源：图片请求在本地找不到文件时，自动按图片记录从所属储存（本地 / WebDAV / S3 等）读取并输出，任意储存间迁移后旧链接均可正常访问，同时写入本地缓存加速后续访问
- 社会化登录：兼容聚合 CC 登录接口（appid/appkey 方式），可在后台配置
